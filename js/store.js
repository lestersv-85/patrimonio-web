...
import { CONFIG } from './config.js?v=57b99b9';
import { toast, uid } from './util.js?v=57b99b9';
...
export async function loadAll() {
  ...
  if (client) {
    ...
    const { accounts, positions, operations, dividends, clock, hist, settings, syncLog, prices, fx, bench, macro, watchlist } = got;
    Object.assign(DB, { accounts, positions, operations, dividends, clock, filiosHistory: hist, settings, syncLog, watchlist: watchlist || [], prices: compactToMap(prices, 'position_id'), fx: compactToMap(fx, 'ccy'), bench: compactToMap(bench, 'symbol'), macro: compactToMap(macro, 'series') });
