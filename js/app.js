...
import { DB, loadAll, signIn, signInWithLink, signOut, localEditCount } from './store.js?v=3bf8a9f';
import { $, $$, esc, todayISO, fmtDate, toast, BUCKETS, TYPES, HELP } from './util.js?v=3bf8a9f';
import { SEL, selIsAll, compute, invalidate } from './engine.js?v=3bf8a9f';
import { setupChartDefaults, destroyCharts } from './charts.js?v=3bf8a9f';
import { phaseHeaderHTML, phaseLineHTML } from './clock.js?v=3bf8a9f';
import { recommendationsHTML } from './recommend.js?v=3bf8a9f';
import { renderInicio } from './views/inicio.js?v=3bf8a9f';
import { renderCartera } from './views/cartera.js?v=3bf8a9f';
import { renderMovimientos } from './views/movimientos.js?v=3bf8a9f';
import { renderRendimientos } from './views/rendimientos.js?v=3bf8a9f';
import { renderDistribucion } from './views/distribucion.js?v=3bf8a9f';
import { renderPosiciones } from './views/posiciones.js?v=3bf8a9f';
import { renderCuentas } from './views/cuentas.js?v=3bf8a9f';
import { renderAjustes } from './views/ajustes.js?v=3bf8a9f';
import { renderImpuestos } from './views/impuestos.js?v=3bf8a9f';
import { renderEstrategia } from './views/estrategia.js?v=3bf8a9f';
import { renderPatrimonio } from './views/patrimonio.js?v=3bf8a9f';
import { renderAnalisis } from './views/analisis.js?v=3bf8a9f';
import { renderReferencia } from './views/referencia.js?v=3bf8a9f';
import { renderTaller } from './views/taller.js?v=3bf8a9f';
import { renderSeguimiento } from './views/seguimiento.js?v=3bf8a9f';
import { hacerOrdenables } from './sortable.js?v=3bf8a9f';
import { runChecks, dismissedKeys, dismiss, undismissAll } from './checks.js?v=3bf8a9f';
import { openModal, closeModal } from './forms.js?v=3bf8a9f';
import { openOpForm, setRerender } from './forms.js?v=3bf8a9f';
...
const ICONS = {
  ...
  tamanos: '<svg viewBox="0 0 24 24"><path d="M4 20V10M10 20V4M16 20v-7M22 20V8"/></svg>',
  seguimiento: '<svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"/><path d="M20 20l-3.5-3.5"/><path d="M11 8v6M8 11h6"/></svg>',
  mas: '<svg viewBox="0 0 24 24"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>',
};
const NAV = [['inicio', 'Inicio', 'both'], ['cartera', 'Cartera', 'desk'], ['patrimonio', 'Patrimonio', 'desk'], ['movimientos', 'Movimientos', 'both'], ['rendimientos', 'Rendimientos', 'both'], ['distribucion', 'Distribución', 'both'], ['analisis', 'Análisis', 'desk'], ['posiciones', 'Posiciones', 'desk'], ['cuentas', 'Cuentas', 'desk'], ['impuestos', 'Impuestos', 'desk'], ['estrategia', 'Estrategia', 'desk'], ['referencia', 'Referencia', 'desk'], ['tamanos', 'Dimensionado', 'desk'], ['seguimiento', 'Seguimiento', 'desk'], ['ajustes', 'Ajustes', 'desk'], ['mas', 'Más', 'mob']];
const MAS_VIEWS = ['cartera', 'patrimonio', 'analisis', 'posiciones', 'cuentas', 'impuestos', 'estrategia', 'referencia', 'tamanos', 'seguimiento', 'ajustes'];
...
export function render() {
  ...
  try {
    ...
    hacerOrdenables(v);   // todas las tablas se ordenan pulsando su cabecera
...
// ---- Guardián de ancho (solo en local) ----------------------------------------------------------
// La web solo debe desplazarse hacia abajo. Una caja que se pase de ancho mueve la página de lado y
// es un fallo difícil de ver en el Mac y evidente en el iPhone, así que se avisa donde se desarrolla.
// `#view` lleva `overflow-x: clip`, que ESCONDE el desbordamiento en los navegadores que lo soportan:
// por eso el guardián lo desactiva un instante para medir de verdad. No corre en producción.
if (new URLSearchParams(location.search).has('local')) {
  const mirarAncho = () => {
    const de = document.documentElement, v = $('#view');
    const antes = v ? v.style.overflowX : null;
    if (v) v.style.overflowX = 'visible';
    const sobra = de.scrollWidth - de.clientWidth;
    let culpable = null;
    if (sobra > 0) {
      let peor = 1;
      for (const el of document.querySelectorAll('body *')) {
        ...
        const ex = b.right - de.clientWidth;
        if (ex <= peor) continue;
        let recortado = false;
        for (let p = el.parentElement; p && p !== document.body; p = p.parentElement) {
          const o = getComputedStyle(p).overflowX;
          if (o === 'auto' || o === 'scroll' || o === 'hidden' || o === 'clip') { recortado = true; break; }
        }
        if (!recortado) { peor = ex; culpable = el; }
      }
    }
    if (v) v.style.overflowX = antes || '';
    if (sobra > 0) {
      console.warn(`[ancho] La página se mueve ${Math.round(sobra)} px de lado a ${de.clientWidth} px. ` +
        `Solo debería desplazarse hacia abajo.`, culpable || '(dentro de un contenedor con scroll)');
    }
  };
  addEventListener('resize', () => { clearTimeout(window.__anchoT); window.__anchoT = setTimeout(mirarAncho, 250); });
  setTimeout(mirarAncho, 1200);
  addEventListener('hashchange', () => setTimeout(mirarAncho, 900));
}
