...
import { $, $$, esc, fmtCcy, fmtDate, TYPES, DRAWERS, ESTILOS} from '../util.js?v=3bf8a9f';
import { defaultBucket, priceAt } from '../engine.js?v=3bf8a9f';
import { openPosForm } from '../forms.js?v=3bf8a9f';
...
export function renderPosiciones(v, C, { UI, render }) {
  ...
  v.innerHTML = `
    ...
    <div class="card"><div class="tablewrap"><table><thead><tr><th>Ticker</th><th>Nombre</th><th>Tipo</th><th>Cubo</th><th>Cajón</th><th>Estilo</th><th>Región</th><th>Divisa</th><th class="num">Último precio</th><th>Fecha</th><th>Fuente</th><th>Sector</th></tr></thead><tbody>
      ${list.map(p => { const pr = priceAt(p.id, today); return `<tr class="row" data-pos="${p.id}"><td><span class="tk">${esc(p.ticker)}</span>${p.scope === 'vivienda' ? ' <span class="tag">vivienda</span>' : ''}${held.has(p.id) ? '' : ' <span class="tag">cerrada</span>'}</td><td>${esc(p.name)}</td><td>${TYPES[p.type] || esc(p.type)}</td><td><span class="tag b${defaultBucket(p)}">${defaultBucket(p)}</span></td><td class="muted">${p.drawer ? DRAWERS[p.drawer] : ''}${p.nucleo_sector ? ' · ' + esc(p.nucleo_sector) : ''}</td><td class="muted">${p.estilo ? esc(ESTILOS[p.estilo] || p.estilo) : ''}</td><td class="muted">${esc(p.region || '')}</td><td>${esc(p.currency)}</td><td class="num">${pr ? fmtCcy(pr.price, p.currency, pr.price < 1 ? 4 : 2) : '—'}</td><td class="muted">${pr ? fmtDate(pr.date) : '—'}${pr && pr.stale && p.price_mode !== 'manual' && held.has(p.id) ? ' <span class="tag stale">antiguo</span>' : ''}</td><td class="muted">${p.price_mode === 'manual' ? 'manual' : esc(p.yahoo_symbol || p.coingecko_id || 'sin símbolo')}</td><td class="muted">${esc(p.sector || '')}</td></tr>`; }).join('') || `<tr><td colspan="12" class="empty">Sin posiciones.</td></tr>`}
    </tbody></table></div></div>`;
