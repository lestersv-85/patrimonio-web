...
import { DB, saveSettings } from '../store.js?v=3bf8a9f';
import { $, $$, esc, fmtEUR, fmtK, fmtN, fmtPct, fmtDate, cls, sum, todayISO, daysBetween, BUCKETS, PHASES, HELP, help, toast, num } from '../util.js?v=3bf8a9f';
import { inSelPos, inSelCash, selIsAll, viviendaValue, expandirCubos, saludComposiciones } from '../engine.js?v=3bf8a9f';
import { phaseHeaderHTML, currentPhase } from '../clock.js?v=3bf8a9f';
import { recommendations, targets } from '../recommend.js?v=3bf8a9f';
...
export function renderEstrategia(v, C, { UI, render }) {
  ...
  // Se mira DENTRO de los fondos: un mixto reparte su valor entre los cubos que de verdad lleva.
  const byB = { 1: 0, 2: 0, 3: cash, 4: 0 }; expandirCubos(rows).forEach(r => byB[r.bucket] += r.mvEUR);
  const SAL = saludComposiciones(rows);
  ...
  v.innerHTML = `
    ...
    <div class="grid2">
      ...
      <div class="card pad"><h3>Los cuatro cubos frente a la doctrina${selIsAll() ? '' : ' · <span class="warn">con filtro</span>'}</h3>
        ${selIsAll() ? '' : `<div class="note warn"><b>Hay un filtro activo, así que la columna «Actual» no es la de tu cartera</b> sino la del trozo filtrado. Los objetivos son del conjunto: comparar contra ellos una parte da una desviación falsa. Quita el filtro para leer esta tabla.</div>`}<div class="tablewrap"><table><thead><tr><th>Cubo</th><th class="num">Actual</th><th class="num">Objetivo</th><th class="num">Desviación</th><th class="num">Importe</th></tr></thead><tbody>
