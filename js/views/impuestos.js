...
import { $, $$, esc, fmtEUR, fmtK, fmtN, fmtPct, fmtDate, cls, sum, todayISO, addDays, TYPES, help, toast } from '../util.js?v=3bf8a9f';
import { replay, compute, opAmounts, divAmounts, posOf, acctOf, acctName, isInvest } from '../engine.js?v=3bf8a9f';
  ...
  v.innerHTML = `
    ...
    <div class="grid2">
      <div class="card"><div class="head">${help('Modelo 720 y 721', 'Obligación informativa (no se paga) si a 31 de diciembre tienes más de 50.000 € en el extranjero en alguna categoría: cuentas, valores (720) o criptomonedas (721). Se calcula por país de la cuenta: Trade Republic es Alemania, Nexo es Francia.')}</div><div class="pad" style="padding:.6rem 1.1rem 1rem">
        ${Object.entries(m720.totals).map(([cat, t]) => `<div style="display:flex;flex-wrap:wrap;justify-content:space-between;gap:.2rem .6rem;padding:.3rem 0;border-bottom:1px solid var(--line)"><span style="min-width:0">${cat}</span><span class="num ${t > 50000 ? 'warn' : ''}" style="min-width:0;text-align:right">${fmtEUR(t, 0)} ${t > 50000 ? '· obligatorio' : '· por debajo de 50 k€'}</span></div>`).join('') || '<div class="muted">Sin cuentas fuera de España.</div>'}
