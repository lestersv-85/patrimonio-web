...
import { $, $$, esc, fmtEUR, fmtK, fmtN, cls, sum, BUCKETS, TYPES, DRAWERS, PALETTE, C as COL, help, typeColor, accountColor, ccyColor, ESTILOS, ESTILO_COLOR} from '../util.js?v=3bf8a9f';
import { inSelPos, inSelCash, selIsAll, acctName, groupBy, viviendaValue, expandirCubos, saludComposiciones } from '../engine.js?v=3bf8a9f';
import { donut, legendHTML } from '../charts.js?v=3bf8a9f';
import { openModal } from '../forms.js?v=3bf8a9f';
import { selDescription } from './inicio.js?v=3bf8a9f';
import { targets } from '../recommend.js?v=3bf8a9f';
...
export function renderDistribucion(v, C, { UI, render, filterBarHTML, bindFilterBar }) {
  ...
  // Solo el reparto POR CUBO desdobla los fondos mixtos; en tipo, región o sector desdoblar
  // duplicaría la fila y contaría dos veces el mismo dinero.
  const byBucket = groupBy(expandirCubos(rows).map(r => ({ ...r, mvEUR: val(r) })), r => r.bucket, k => BUCKETS[k].name, k => BUCKETS[k].color, PALETTE);
  const SAL = saludComposiciones(rows); if (cash) byBucket.push({ k: 'cash', label: 'Efectivo', v: cash, color: COL.cash, cost: cash, gain: 0, n: DB.accounts.length });
  ...
  // Estilo: cómo gana dinero cada posición. Va sobre toda la cartera, no solo el cubo 1, para que
  // «Sin estilo» enseñe lo que falta por clasificar en vez de esconderlo.
  // Solo lo que puede tener estilo: acciones, ETF, fondos y cripto. Las casas, Reental y el plan no
  // ganan dinero de ninguna de esas maneras, y meterlos haría que «Sin estilo» fuera el 63 % y no
  // dijera nada. Así, «Sin estilo» es exactamente lo que queda por clasificar.
  const conEstilo = rows.filter(r => ['stock', 'etf', 'fund', 'crypto'].includes(r.p.type));
  const byEstilo = groupBy(conEstilo.map(r => ({ ...r, mvEUR: val(r) })), r => r.p.estilo || '',
    k => ESTILOS[k] || k || 'Sin estilo', k => ESTILO_COLOR[k] || ESTILO_COLOR[''], PALETTE);
  const b1 = rows.filter(r => r.bucket === 1); const byDrawer = groupBy(b1.map(r => ({ ...r, mvEUR: val(r) })), r => r.p.drawer || '', k => (k ? (DRAWERS[k] || k) : 'Sin cajón'), null, PALETTE);
  ...
  const HK = { 'd-b': 'cubo', 'd-dr': 'cajones', 'd-e': 'estilo', 'd-ns': 'nucleo', 'd-t': 'tipo', 'd-r': 'region', 'd-s': 'sector', 'd-a': 'cuenta', 'd-c': 'divisa' };
  ...
  const tables = { 'd-b': ['Por cubo', byBucket], 'd-dr': ['Cajones del cubo 1', byDrawer], 'd-e': ['Por estilo', byEstilo], 'd-ns': ['Sectores del núcleo', byNucSector], 'd-t': ['Por tipo de activo', byType], 'd-r': ['Por región', byRegion], 'd-s': ['Por sector', bySector], 'd-a': ['Por cuenta', byAcc], 'd-c': ['Por divisa', byCcy] };
  v.innerHTML = `
    ...
    <div class="card"><div class="head"><h3>Los cuatro cubos</h3><span class="sub">Peso sobre lo rebalanceable frente al objetivo de la doctrina${selIsAll() ? '' : ' · <b class="warn">con un filtro puesto</b>'}</span></div>
      ${selIsAll() ? '' : `<div class="note warn" style="margin:0 1rem .6rem"><b>Hay un filtro activo (${esc(selDescription())}), así que estos pesos NO son los de tu cartera.</b> Los objetivos de la doctrina son del conjunto, y comparar contra ellos un trozo filtrado da un resultado falso: si filtras por acciones, el cubo 1 sale al 100 % y los otros tres a cero, porque son justo lo que el filtro ha quitado. Por eso aquí no se dice si estás dentro o fuera de banda. Quita el filtro de arriba para ver el reparto de verdad.</div>`}
      <div class="cubos">${[1, 2, 3, 4].map(b => { const vv = byBucket.filter(x => x.k === b || (b === 3 && x.k === 'cash')).reduce((s, x) => s + x.v, 0); const w = b === 4 ? vv / (total || 1) * 100 : vv / (reb || 1) * 100; const t = T.cubos_rebalanceable[b]; const band = T.banda_pp; const mx = 100; return `<div class="kpi" style="--kc:${BUCKETS[b].color}"><div class="l" style="color:${BUCKETS[b].color}">${BUCKETS[b].short}</div><div class="v">${fmtN(w, 1)} %</div>${b !== 4 ? `<div class="tgt" title="Peso actual (barra), objetivo (marca) y banda tolerada ±${band} pp (zona clara)"><div class="tgt-band" style="left:${Math.max(0, t - band) / mx * 100}%;width:${2 * band / mx * 100}%"></div><div class="tgt-bar" style="width:${Math.min(100, w) / mx * 100}%;background:${BUCKETS[b].color}"></div><div class="tgt-mark" style="left:${t / mx * 100}%"></div></div>` : ''}<div class="s">${fmtK(vv)} · ${b === 4 ? 'del total' : `objetivo ${fmtN(t, 1)} %${selIsAll() ? ` · ${w - t >= 0 ? '+' : ''}${fmtN(w - t, 1)} pp${Math.abs(w - t) > band ? ' · fuera de banda' : ' · en banda'}` : ' · sin comparar: hay filtro'}`} · ${BUCKETS[b].job}</div></div>`; }).join('')}</div></div>
    <div class="grid2">
      ${card('d-b', 'Por cubo', byBucket)}${card('d-dr', 'Cajones del cubo 1', byDrawer, 'Por regla de venta')}${card('d-e', 'Por estilo', byEstilo, 'Solo acciones, fondos y cripto')}${card('d-ns', 'Sectores del núcleo', byNucSector, 'Objetivo 8 sectores')}${card('d-t', 'Por tipo de activo', byType)}${card('d-r', 'Por región', byRegion)}${card('d-s', 'Por sector', bySector)}${card('d-a', 'Por cuenta', byAcc)}${card('d-c', 'Por divisa', byCcy)}
    </div>`;
  bindFilterBar();
