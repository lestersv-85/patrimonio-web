import { DB, saveSettings, localEditCount, clearLocalEdits, signOut, setPassword } from '../store.js?v=3bf8a9f';
import { $, $$, esc, fmtN, fmtDate, toast, sum, PHASES } from '../util.js?v=3bf8a9f';
import { fxAt, latestAtOrBefore } from '../engine.js?v=3bf8a9f';
import { targets, DEFAULT_TARGETS } from '../recommend.js?v=3bf8a9f';
import { exportAllXLSX, exportAllCSV, exportOpsCSV, exportDivsCSV, exportPositionsCSV, exportCatalogCSV, exportParaSkills } from '../export.js?v=3bf8a9f';
import { importFiliosCSV } from '../forms.js?v=3bf8a9f';
...
export function renderAjustes(v, C, { render, go }) {
  ...
  v.innerHTML = `
    ...
    <div class="card pad" style="display:grid;gap:.6rem"><h3>Objetivos de la doctrina</h3>
      <div class="form">
        ...
        <div class="field"><label>Bitcoin (% base)</label><input type="number" step="0.5" id="t-btcbase" value="${T.btc_base}"></div>
  ...
  $('#e-skills').onclick = () => { try { exportParaSkills(C); toast('Cinco CSV en formato Filios. Déjalos en la carpeta Filios_Data.'); }
    catch (e) { toast('No exporto: ' + e.message); } };
