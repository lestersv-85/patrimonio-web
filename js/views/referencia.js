...
import { DB, saveSettings } from '../store.js?v=57b99b9';
import { $, $$, esc, fmtN, fmtPct, fmtYM, cls, todayISO, monthKey, addMonths, toast, C as COL } from '../util.js?v=57b99b9';
import { BENCH_LEGS, BENCH_DEFAULT, benchWeights, benchComposite, benchMonthReturn, riskMetrics, nivelRiesgo, spBench, inSelPos, inSelCash, defaultBucket } from '../engine.js?v=57b99b9';
import { chart, grid, pctTick, endLabels } from '../charts.js?v=57b99b9';
...
const PRESETS = [
  { id: 'doctrina', nm: 'Doctrina 60/5/15/10/10', w: { ...BENCH_DEFAULT } },
...
export function renderReferencia(v, C, { UI, render, go }) {
  ...
  v.innerHTML = `<h2>Índice de referencia</h2>
    <p class="muted" style="margin:.2rem 0 .9rem;font-size:.85rem;max-width:78ch">Una cartera con oro, bonos, caja y cripto no se mide contra un índice de acciones. Los botones y el deslizador van de punto en punto; si quieres decimales, escríbelos en la casilla. Mueve los pesos y mira qué riesgo compras con cada reparto: todo se calcula con los rendimientos mensuales reales de cada activo, nunca con estimaciones. <b>Los pesos que guardes son los que dibuja el gráfico de Inicio.</b></p>
  ...
  $('#bm-rows').innerHTML = BENCH_LEGS.map(l => `
    <div class="bm-w" style="--sw:${l.color}">
      ...
      <span class="val"><input type="number" id="bm-n-${l.id}" min="0" max="100" step="1" aria-label="Peso de ${esc(l.nm)} en por ciento"${l.id === 'caja' ? ' disabled' : ''}><span class="muted">%</span></span>
      <input type="range" id="bm-s-${l.id}" min="0" max="100" step="1" aria-label="Ajustar ${esc(l.nm)}"${l.id === 'caja' ? ' disabled' : ''}>
    </div>`).join('');
