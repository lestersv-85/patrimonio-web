...
export const HELP = {
  ...
  tk_div: 'Qué es: el dividendo bruto cobrado en los últimos doce meses, por título y en euros, dividido por el precio de hoy. Qué te dice: la mitad de la rentabilidad que la tesis no mide. La tesis que escribes es de precio; μ tiene que ser retorno total, así que el modelo suma las dos. Ojo al número de pagos que sale al pasar el cursor: si una trimestral enseña dos pagos, faltan cobros en la base y este rendimiento se queda corto.',
  tk_mubonos: 'Qué es: el retorno esperado del cajón de bonos. Qué te dice: nada que el sistema sepa. Es la TIR del fondo a vencimiento, un dato que está en su ficha y no en esta base, así que por defecto entra igualada al tipo sin riesgo, que es el supuesto conservador. La rentabilidad histórica de la serie se enseña al lado etiquetada como historia: no la copies aquí sin pensarlo, que es justo el error contra el que avisa el resto de la pantalla.',
  ...
  tk_margen: 'Qué es: cuánto se aparta el precio del valor que sale de una valoración por fundamentales (DCF sobre owner earnings, o el método que corresponda al negocio: P/B y ROE en un banco o una aseguradora, múltiplo de FFO en un REIT, descuento de dividendos en una utility). Qué te dice: dónde hay margen de seguridad y dónde no. Verde por encima del 30 %, rojo por debajo de −15 %. Abre la ficha de la posición para ver el método, los supuestos y las fuentes: el número solo no vale nada.',
  k_indice: 'Qué es: lo que valdrían tus posiciones si cada euro que metiste en cada una hubiera ido ese MISMO día a un ETF mundial, y hubieras sacado el mismo dinero en las mismas fechas. Qué te dice: si elegir valores te está pagando. Se mide posición a posición, no por cuenta: una cuenta mezcla tu criterio con el dinero que tienes parado en caja, y eso castiga algo que no es elegir bien.',
