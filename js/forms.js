...
import { DB, upsert, remove, savePrice } from './store.js?v=3bf8a9f';
import { $, $$, esc, uid, todayISO, nowHM, fmtEUR, fmtN, fmtPct, fmtCcy, fmtDate, cls, num, toast, TYPES, BUCKETS, DRAWERS, OP_LABEL, sum, ESTILOS} from './util.js?v=3bf8a9f';
import { posOf, acctOf, acctName, fxAt, priceAt, opAmounts, divAmounts, defaultBucket, sortOps, compute, invalidate, replay } from './engine.js?v=3bf8a9f';
...
export function openHolding(key, C) {
  ...
  openModal(`
    ...
    ${(() => {
      // La valoración por fundamentales de esta posición, si la hay. Va AQUÍ, en la ficha, porque es
      // donde surge la pregunta «¿y por qué dice eso?»: el número solo, sin el método ni las fuentes,
      // es lo que este proyecto lleva toda la sesión desmontando.
      const V = DB.settings?.tesis?.[r.p.id]?.valoracion; if (!V) return '';
      const m = +V.margen_base_pct;
      const banda = m >= 30 ? ['g', 'Núcleo'] : m >= -15 ? ['', 'Mantener'] : ['r', 'Candidata a recorte'];
      const esc0 = x => x == null ? null : fmtCcy(+x, r.p.currency, +x < 10 ? 3 : 2);
      const fila = (t, v) => v ? `<dt>${t}</dt><dd>${esc(String(v))}</dd>` : '';
      return `<h3>Valoración por fundamentales <span class="tag ${banda[0] === 'r' ? 'stale' : ''}" style="margin-left:.4rem">${banda[1]}</span></h3>
      <dl class="kv">
        <dt>Valor frente al precio</dt><dd class="num"><b class="${m >= 0 ? 'pos' : 'neg'}">${fmtPct(m, 1)}</b> · valor ${esc0(V.valor_base) || '—'} frente a ${esc0(V.precio) || '—'} de precio</dd>
        ${V.valor_bear != null || V.valor_bull != null ? `<dt>Escenarios</dt><dd class="num">${esc0(V.valor_bear) || '—'} pesimista · <b>${esc0(V.valor_base) || '—'}</b> base · ${esc0(V.valor_bull) || '—'} optimista</dd>` : ''}
        <dt>Método</dt><dd>${esc(V.metodo || '—')}${V.descuento_pct ? ` · descuento ${fmtN(+V.descuento_pct, 0)} %` : ''}${V.crecimientos ? ` · crecimientos ${esc(String(V.crecimientos))}` : ''}${V.crecimientos_bear_base_bull ? ` · crecimientos ${esc(String(V.crecimientos_bear_base_bull))}` : ''}</dd>
        ${fila('Owner earnings', V.owner_earnings)}
        ${fila('Deuda neta', V.deuda_neta)}
        ${fila('Acciones', V.acciones)}
        ${fila('Fuente de los datos', V.fuente_datos)}
        ${fila('Fuente del precio', V.fuente_precio)}
        ${fila('Contraste con otros modelos', V.contraste)}
        ${fila('Nota', V.nota)}
        <dt>Fecha</dt><dd>${fmtDate(V.fecha)}</dd>
      </dl>
      <div class="muted" style="font-size:.74rem;margin:-.2rem 0 .6rem">${esc(V.por_que_no_se_toca_alza || '')} El valor intrínseco es una opinión con aritmética: los supuestos de crecimiento y descuento se eligieron a mano y están escritos arriba. No es un precio objetivo ni una orden.</div>`;
    })()}
...
export function openPosForm(p, { onSaved } = {}) {
  const isNew = !p;
  p = p ? { ...p } : { id: '', ticker: '', name: '', type: 'stock', currency: 'USD', isin: '', country: '', exchange: '', sector: '', region: '', bucket: 1, drawer: '', estilo: '', nucleo_sector: '', scope: 'inversion', price_mode: 'auto', yahoo_symbol: '', coingecko_id: '', tags: [], notes: '' };
  ...
  openModal(`
    ...
    <div class="form">
      ...
      <div class="field"><label>Estilo</label><select id="p-estilo">${opt(Object.keys(ESTILOS), p.estilo || '', k => ESTILOS[k])}</select><span class="hint">Qué tiene que pasar para que suba: compounder, crecimiento, defensiva, cíclica, valor, renta o especulativa. Lo que no es una acción va a RV indexada, renta fija o cripto. Distinto del sector (de qué va) y del cajón (cuándo se vende).</span></div>
